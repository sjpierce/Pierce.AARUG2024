...
edge [fontname="Lato", fontsize="40pt"]
...
* [RStudio Desktop](https://posit.co/download/rstudio-desktop/), an editor
* [Quarto](https://quarto.org), for dynamic documents 
* [Git](https://git-scm.com/), for version control
* [GitHub.com](https://github.com), for sharing & collaborating on code

::: {.notes}
I'm trying to show you a better way to do analyses and write reports. It depends
on using better software and learning new workflows. This is a list of free
software packages that comprise an elegant, powerful, and integrated set of
tools for doing reproducible analyses and reports. These are my favorite tools
now, in part because they enable me to create dynamic documents. R is a fantastic
:::
...
edge [fontname="Lato", fontsize="30pt"]
Data -> Quarto [label = "Read by"]
BibTeX -> Quarto [label = "Entries cited in"]
...
::: {.notes}
So what are dynamic documents? They draw on the concept of literate programming,
where one embeds code directly in a document such as a report, interleaving it
with narrative text, headings, and in-text citations. When you render dynamic
documents, the code is executed and the results are directly inserted into the
output. That includes statistics computed from the data, figures, tables,
formatted equations, fully-formatted reference sections, and more. This diagram 
shows the files involved for a simple example. The idea is that rendering your 
report script produces a PDF file ready for delivery to your client. 
...
analyses. Quarto automates inserting results into the report. I'm using similar 
filenames to stay organized: they show which script creates the report. Saving
...
::: {.notes}
Dynamic documents are a key tool for reproducibility, but they're only part of
the solution. Most projects require more than couple files, so organizing your
materials into a research compendium is useful. A compendium is a structured
folder containing all your reproducibility files. An effective one should use
prevailing conventions to organize the project; keep data, method, and output
separate even while revealing how they are related; and it should tell users
what software was used in the original analysis. 
:::
...
... are folders of files being tracked by Git for version control purposes. 
[@Bryan-RN3900; @Chacon-RN3480] They:

* Preserve a history of changes to the tracked file 
...
::: {.notes}
GitHub.com is an online server that hosts Git repositories, allowing multiple
people to synchronize their local copies with the main one hosted on GitHub.
This facilitates collaboration because each person can independently edit files,
then pull the recent changes others have made down from GitHub, and push their
own changes back up to the main repository. Git will detect conflicting changes
and force a user to resolve them and commit the result.
:::
